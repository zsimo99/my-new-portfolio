...
const Projects = ({ service, showAll, setLoad }) => {
    ...
    return (
        <div className="container mx-auto px-6 projects flex flex-col gap-16 mt-16">
            {!showAll ? (projects.filter(project => project.category === service).length>0?projects.filter(project => project.category === service).slice(0, 5).map((project, id) => <Project key={id} nm={id} project={project} />):<p className="text-center text-gray-400 text-lg md:text-xl"> no projects available in this category. </p>)
                :  (projects.filter(project => project.category === service).map((project, id) => <Project key={id} nm={id} project={project} />))

            }
            {!showAll && projects.filter(project => project.category === service).length > 5 && <>
